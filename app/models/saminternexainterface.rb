class Saminternexainterface < ActiveRecord::Base

  serialize :saminternexainterfaces_array
  ...
  def self.interfacestats_summary
   xml = File.read("#{Rails.root}/public/InterfaceStats_Internexa.xml")
   data = RestClient.post("http://10.140.255.1:8080/xmlapi/invoke", xml,{:"Content-Type" => 'application/soap+xml'})
   data_parsed = Hash.from_xml(data)
   return data_parsed
  end
  ...
  def self.tx_internexa
   speednokia_array = CSV.read(Rails.root + "public/tx_internexa.csv")
  end
  ...
  def self.statsinterfacetotal_table
   ...
   data_stats.map do |qosdiscard_device|
    ...
    tx_internexa.map do |tx_internexa|
      if tx_internexa[0] == devicetotal_porttotal
