class Samutilizationinterface < ActiveRecord::Base

  serialize :samutilizationinterfaces_array
    ...
    def self.statsinterface_table
     ...
     statsinterfacemax_table.map do |stats_device|
      ...
      unless @bw == 0
        ...
        hash = Hash[node: @device, Mbps_bwrx: @Mbps_bwrx, Mbps_bwtx: @Mbps_bwtx, status: @status, velodidad: @actualSpeed, port: @port, description: @description, timeCaptured: @timeCaptured, speed: @speed, deviceport: @device_int_stats]
        table_stats << hash
      end
     end
