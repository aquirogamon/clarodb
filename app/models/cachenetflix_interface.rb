class CachenetflixInterface < ActiveRecord::Base
  ...
  def self.devint
    CSV.read(Rails.root + "public/netflix.csv")[1..-1]
  end
  ...
  def self.statsinterfacecrecimiento_table
   ...
   statsinterfacetotal_table.map do |statstotal|
    ...
    if statstotal[:status_total] > 100.00
      ...
      (CachenetflixInterface.where(CachenetflixInterface.arel_table[:created_at].gt(CachenetflixInterface.maximum(:created_at).to_date))).map do |crecimiento|
