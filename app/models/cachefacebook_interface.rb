class CachefacebookInterface < ActiveRecord::Base
  ...
  def self.devint
    CSV.read(Rails.root + "public/facebook.csv")[1..-1]
  end
  ...
  def self.statsinterfacetotal_table
    ...
    devint.each { 
      ...
      keystring = a + c
      extra_data[keystring] = Hash[bw: d.to_f, nodo_total: e.to_s, router_total: f.to_s]
    }
      ...
      interfaces[device[0]].each do |interface|
        ...
        if @table[keystring] != []
          ...
          hash2 = Hash[device_total: device[0], 
            ...
            nodo_total: extra_data[keystring][:nodo_total], 
  ...
  def self.statsinterfacecrecimiento_table
   ...
   statsinterfacetotal_table.map do |statstotal|
    ...
    if statstotal[:status_total] > 100.00
      ...
      (CachefacebookInterface.where(CachefacebookInterface.arel_table[:created_at].gt(CachefacebookInterface.maximum(:created_at).to_date))).map do |crecimiento|
