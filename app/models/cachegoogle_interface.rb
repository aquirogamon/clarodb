class CachegoogleInterface < ActiveRecord::Base
  ...
  def self.devint
    CSV.read(Rails.root + "public/google.csv")[1..-1]
  end
  ...
  def self.statsinterfacecrecimiento_table
   ...
   statsinterfacetotal_table.map do |statstotal|
    ...
    if statstotal[:status_total] > 100.00
      ...
      (CachegoogleInterface.where(CachegoogleInterface.arel_table[:created_at].gt(CachegoogleInterface.maximum(:created_at).to_date))).map do |crecimiento|
