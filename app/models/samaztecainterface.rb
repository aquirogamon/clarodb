class Samaztecainterface < ActiveRecord::Base

  serialize :samaztecainterfaces_array
  ...
  def self.interfacestats_summary
   xml = File.read("#{Rails.root}/public/InterfaceStats_Azteca.xml")
   data = RestClient.post("http://10.140.255.1:8080/xmlapi/invoke", xml,{:"Content-Type" => 'application/soap+xml'})
   data_parsed = Hash.from_xml(data)
   return data_parsed
  end
  ...
  def self.tx_azteca
   speednokia_array = CSV.read(Rails.root + "public/tx_azteca.csv")
  end
  ...
  def self.statsinterfacetotal_table
   ...
   data_stats.map do |qosdiscard_device|
    ...
    tx_azteca.map do |tx_azteca|
      if tx_azteca[0] == devicetotal_porttotal
