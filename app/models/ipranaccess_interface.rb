class IpranaccessInterface < ActiveRecord::Base
  ...
  def self.statsinterfacecrecimiento_table
   ...
   statsinterface_table.map do |statstotal|
    ...
    if (statstotal[:status_total]).to_f > 100.00
      ...
      (IpranaccessInterface.where(IpranaccessInterface.arel_table[:created_at].gt(IpranaccessInterface.maximum(:created_at).to_date))).map do |crecimiento|
