  ...
  def self.statsinterfacetotal_table
    ...
    devint.each { 
      ...
      keystring = a + c
      extra_data[keystring] = Hash[servicio_total: d.to_s]
    }
    ...
    devices.each do |device|
      interfaces[device[0]].each do |interface|
        ...
        if table[keystring] != []
          ...
          maxstatustx = (maxtx/egresstotal).round
          maxstatusrx = (maxrx/egresstotal).round
...
statsinterfacecrecimiento_table


  def self.transport_stats
    base_url = "http://172.19.216.20:8000/api/json/device/getInterfaceGraphs?apiKey=96437ef89ce623f8bc6f5f89e82876c1&interfaceName=IF-192.168.252.100-16880"
    begin
      data = RestClient::Request.execute(:url => base_url , :method => :get, :verify_ssl => false)
    rescue RestClient::ExceptionWithResponse => err
      err.response
...
maxrx = ((transport_stats["traffic"]["Rx Traffic"].sort { |a,b| a["y"] <=> b["y"] }.last["y"])/1000000000).round(2)
maxtx = ((transport_stats["traffic"]["Tx Traffic"].sort { |a,b| a["y"] <=> b["y"] }.last["y"])/1000000000).round(2)
maxstatusrx = (transport_stats["utilization"]["Rx Utilization"].sort { |a,b| a["y"] <=> b["y"] }.last["y"]).round
maxstatustx = (transport_stats["utilization"]["Tx Utilization"].sort { |a,b| a["y"] <=> b["y"] }.last["y"]).round
